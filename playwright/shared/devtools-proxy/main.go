...
// though the browser is launched with --remote-debugging-port=0 (chromedriver
// or Playwright launchServer), so the browser picks a RANDOM ephemeral port.
//
// The proxy is self-sufficient — it needs no cooperation from a wrapper:
//   - Port discovery: it reads the browser's own DevToolsActivePort file,
//     located via /proc (the running browser process' --user-data-dir) with a
//     /tmp glob fallback.
